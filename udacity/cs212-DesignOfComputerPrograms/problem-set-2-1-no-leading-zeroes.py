...
def compile_formula(formula, verbose=False):
    ...
    body = ''.join(tokens)
    formula_tokens = re.split('([A-Z]+)', formula)
    filter_chars_for_0 = ','.join(['{} != 0'.format(w[0]) for w in formula_tokens if w.isupper() and len(w) > 1])
    f = "lambda {}: all([{},{}])".format(parms, body, filter_chars_for_0)
    
    if verbose:
        print "f: {}, \nletters: {}, \nparms: {}, \ntokens: {}, \nbody: {}\n\n\n".format(f, letters, parms, tokens, body)
